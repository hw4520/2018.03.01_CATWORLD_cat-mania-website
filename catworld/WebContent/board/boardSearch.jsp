...
if(startPage>pageBlock){
			%><a href="boardSearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=encodeStr%>">[이전]</a><%
		}
for(int i=startPage;i<=endPage;i++) {
	%><a href="boardSearch.jsp?pageNum=<%=i%>&search=<%=encodeStr%>"><%="["+i+"]" %></a><%
}
if(pageCount>endPage){
	%><a href="boardSearch.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=encodeStr%>">[다음]</a><%
}
}
%>
</div>
