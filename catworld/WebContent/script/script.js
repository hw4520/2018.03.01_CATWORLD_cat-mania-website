...
function pass_check(a,b){
	 var c ="http://localhost:8080/catworld/images/passcheck/"+b+".jpg"
	 var c = c.toLowerCase(); // 문자열을 소문자로 변환합니다.
	 console.log(c);
	 var c = c.replace(/(\s*)/g,"")
	 console.log(c); // 모든 공백을 제거	 	  
	if(a==c){
		alert("정답입니다");
		location.href="sessionclear.jsp";
	}else{alert("틀렸습니다.");
		++document.fr.h.value;
		if(document.fr.h.value==5){
		location.href="../contact/contactForm.jsp";
		}
	}
}
...
function emailsend(email1, email2){
    // 유효성 검사
	if(!insertform.email1.value || !insertform.email2.value){ 
		alert(emailerror);
		insertform.email1.focus();
