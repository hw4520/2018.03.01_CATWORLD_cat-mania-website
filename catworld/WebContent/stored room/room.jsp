...
// boardList = getBoardList(시작행번호, 몇개)함수 호출
	...
	roomList= rdao.getRoomList(startRow, pageSize);
%>
<article>
...
if(startPage>pageBlock){
			%><a href="room.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a><%
		}
for(int i=startPage;i<=endPage;i++) {
	%><a href="room.jsp?pageNum=<%=i%>"><%="["+i+"]" %></a><%
}
if(pageCount>endPage){
	%><a href="room.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a><%
}
}
%>
</div>
