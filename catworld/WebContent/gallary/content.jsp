...
<tr><td>글번호</td><td><%=gb.getNum() %></td><td>조회수</td><td><%=gb.getReadcount()%></td></tr>
<tr><td>작성자</td><td><%=gb.getName() %></td><td>작성일</td><td><%=gb.getDate() %></td></tr>
<tr><td>글제목</td><td colspan="3"><%=gb.getSubject() %></td></tr>
<tr><td>첨부파일</td><td colspan="3"><a href="file_down.jsp?file_name=<%=encodeStr%>"><%=gb.getFile()%></a></td></tr>
<!-- 갤러리 게시판은 <img src="../upload/<%=gb.getFile()%>" width="50" height="50"> -->
<tr><td>글내용</td><td colspan="3"><%=content%></td></tr>
<tr><td colspan="4">
<% 
if(s!=null){
  if(s.equals(gb.getName())){
%>
<input type="button" value="글수정" onclick="location.href='updateForm.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'">
<input type="button" value="글삭제" onclick="location.href='deleteForm.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'">
<%
  }
}
%>
<input type="button" value="글목록" onclick="location.href='gallary.jsp?pageNum=<%=pageNum%>'"></td></tr>
</table>
</article>
<!-- 본문내용 -->
<!-- 본문들어가는 곳 -->
