...
int count = gdao.getGallaryCount(search);
...
gallaryList= gdao.getGallaryList(startRow, pageSize, search);
%>
<article>
...
if(startPage>pageBlock){
			%><a href="gallarySearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=encodeStr%>">[이전]</a><%
		}
for(int i=startPage;i<=endPage;i++) {
	%><a href="gallarySearch.jsp?pageNum=<%=i%>&search=<%=encodeStr%>"><%="["+i+"]" %></a><%
}
if(pageCount>endPage){
	%><a href="gallarySearch.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=encodeStr%>">[다음]</a><%
}
}
%>
</div>
